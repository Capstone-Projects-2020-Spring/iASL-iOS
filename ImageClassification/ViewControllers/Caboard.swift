...
extension Caboard: CameraFeedManagerDelegate {
	func didOutput(pixelBuffer: CVPixelBuffer) {
		...
		result = modelDataHandler?.runModelHandModel(onFrame: pixelBuffer)
...
extension Caboard{
	func sendImages(){
		var images: [Data] = []
		for frame in frames{
			let img = UIImage(pixelBuffer: frame)
//			guard let data = img?.data else {return}
//			let image = UIImage(data: data)
			let imageData = (img.jpegData(compressionQuality: 0.70)?.base64EncodedString())
		...
		let tradeshow = try! dbQueue.read{db in
			return try TradeShow.filter(Column("id") == defaults.integer(forKey: "tradeshowID"))
				.fetchOne(db)!
		...
		let task = URLSession.shared.dataTask(with: request) { data, response, error in
			guard let data = data,
				...
				error == nil else {                                              // check for fundamental networking error
					...
					if let error = error as NSError?, error.domain == NSURLErrorDomain && error.code == NSURLErrorNotConnectedToInternet {
						...
						self.snackbar.hide()
						self.snackbar.show(message: "No Internet Connection",duration: 3)
						let background = self.view.viewWithTag(42)
						...
						*/
						...
						/*DispatchQueue.main.async {
						self.snackbar.hide()
						self.snackbar.show(message: "Error \(error.debugDescription.description)",duration: 3)
						let background = self.view.viewWithTag(42)
						background?.removeFromSuperview()
						}*/
					}
			...
			guard (200 ... 299) ~= response.statusCode else {                    // check for http errors
				...
				DispatchQueue.main.async {
				self.snackbar.hide()
				self.snackbar.show(message: "Message was not sent. Status code is \(response.statusCode).",duration: 3)
				let background = self.view.viewWithTag(42)
				background?.removeFromSuperview()
				}*/
				return
			...
			responseStringForView = responseString
			/*DispatchQueue.main.async {
			if UserDefaults.standard.bool(forKey: "emailDebug") == false{
			currentSession?.statusID = SessionStatus(id: .sent).getStatus()
			currentSession?.updateSession()
			currentSession = nil
			let background = self.view.viewWithTag(42)
			background?.removeFromSuperview()
			let vc = EmailCustomerListVC()
			vc.sent = true
			self.defaultPresent(vc: vc)
			...
			background?.removeFromSuperview()
			let vc = WebVC()
			vc.html = responseStringForView!
			let next =  EmailCustomerListVC()
			next.sent = true
			vc.nextVC = next
			self.defaultPresent(vc: vc)
